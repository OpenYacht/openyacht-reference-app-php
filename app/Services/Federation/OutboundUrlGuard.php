...
/**
 * SSRF guard for every server-side federation fetch (well-known
 * discovery, signed sync, media import). A federation host arrives from
 * an unauthenticated header, a partner's discovery document, or a
 * partner's listing payload, so it is untrusted and must never be able
 * to steer a request at internal infrastructure.
 *
 * Two layers:
 *   1. Shape — a bare DNS hostname only. No scheme, userinfo, port, or
 *      path (which blocks `127.0.0.1:6379`, `host:8080/x?`, `a@b`), and
 *      no IP literal (which blocks `169.254.169.254`, `[::1]`).
 ...
 *      not special-cased.
 *
 * Residual: DNS rebinding (a name public at check, private at connect)
 * is not closed here; pinning the resolved IP into the socket would be
 * the next step. Redirects are disabled at each call site so a public
 * host cannot bounce the request to a private one.
 *
...
class OutboundUrlGuard
{
    ...
    private function resolve(string $host): array
    {
        ...
        $ipv6 = collect(@dns_get_record($host, DNS_AAAA) ?: [])
            ->pluck('ipv6')
            ->filter()
            ->all();
