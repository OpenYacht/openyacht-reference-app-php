 ...
 * rendering. Consumers MUST sanitise descriptions regardless of what the
 * authority sent (LS-5): allowed elements are kept, dangerous elements are
 * removed with their content, everything else is unwrapped to its text,
 * and the only attribute that survives is an https href on links.
 *
...
class RichTextSanitizer
{
    ...
    public function sanitize(string $html, array $stripLinkHosts = []): string
    {
        $this->stripLinkHosts = array_map(
            fn (string $host): string => strtolower(preg_replace('/^www\./', '', $host) ?? $host),
            $stripLinkHosts,
        );
    ...
    private function sanitizeChildren(DOMNode $node): void
    {
        ...
        foreach (iterator_to_array($node->childNodes) as $child) {
            ...
            // verbatim lets `<!--><img onerror=…>-->` smuggle a live
            // element past the allow-list on some PHP versions (mXSS).
