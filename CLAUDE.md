...
- **Roles are the fixed five for now, but design for more**: the five-role hierarchy comes from the reference plan, and the roles page manages permissions only. Custom roles are expected eventually — keep authorization permission-based (never `hasRole()` checks in new code) so adding role CRUD later is a UI change, not a refactor.
- **Translation-ready, English-only for now**: not all brokerages speak English. Every user-facing string goes through translation keys (`lang/en/*.php`, `__()` server-side; UI copy passed to Inertia pages as translated props). No hardcoded user-facing text in PHP. Additional locales are a later milestone — the discipline is what matters now.

- **Always latest stable everything**: Laravel 13, PHP 8.4+, current majors of every package. Version drift found at any time is resolved upward, never worked around.
- **Cross-database migrations (CRITICAL)**: SQLite is the zero-config default; MySQL 8 and MariaDB must also work. Every migration must be compatible with all three. Watch the known traps: `->change()` on SQLite, enum alterations, JSON column defaults, DB-specific index/constraint syntax. The test suite runs on SQLite; changes touching schema must also be verified against MySQL/MariaDB before release.
- Copies of partner listings are stored in their own table and are **never re-served** (`/listings` output = own listings only).
- Registry (`registry/builders.json`) is vendored, never fetched at request time.
